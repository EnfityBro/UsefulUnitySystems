...
<br>Assets list:
1. AutoDeviceSimulator - this script automatically creates a XR Device Simulator object on your game scene only in Unity editor mode (for AR/MR/VR games).
2. ControllerVibration - this script allows you to use the controller vibration in the game (this script has been tested on Oculus Quest 2 and Meta Quest 3 VR controllers).
3. Following - with this script, you can make one game object repeat the position and rotation of another game object.
4. ForMenus - useful methods for game menus.
5. FPSCount - a simple FPS counter.
6. LanguageText - This script allows you to create a simple text translation system for other languages in your game.
7. NearestPointOfHit - this is how to track the contact point when one object hits another object.
8. NuclearOptimization - this method disable all mesh renderers, when it called.
...
16. GraphicRaycasterWithLock - using this script instead of the standard GraphicRaycaster component on World Space type Canvases allows you to disable unnecessary mouse processing if it's disabled in the game. This is especially useful in VR games, where interaction takes place through the rays of the controllers.
